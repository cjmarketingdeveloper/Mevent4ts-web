...
function TradeshowRsvp({user, CONSTANTS}) {
    ...
    const [selectedDate, setSelectedDate]                = useState(user.details)
    const optionDesignation                               = ["Buyer", "Pharmacy Manager", "Responsible Pharmacist"];
    const optionDate                                      = ["Friday, 15 May", "Saturday 16 May", "Friday & Saturday, 15-16 May"];


    //////////////Events
    const tradeshowCode    = "69385"; //Tradeshow
    ...
    const handlePersonalSave = async () => {
        try {
          ...
          if (selectedDate !== user.details) {
            updateData.details = selectedDate;
          }
    ...
    const handleEventsSave = async () => {
        try{
              const updateData = {};
              //check if tradeshowCode code is found in user.events. If not found replace every event and have
              //this event only tradeshowCode
              if (tradeshowCode !== user.events) {
                updateData.events = tradeshowCode;
              }
                ...
                <div className="row">
                  <div className="col-md-6 mb-3">
                    ...
                    <select
                      ...
                      onChange={(e) => setUpdateProfession(e.target.value)}
                    >
                      <option value="">-- Select Profession --</option>
                      {optionDesignation.map((role) => (
                        <option key={role} value={role}>{role}</option>
                      ))}
                    </select>
                  </div>
                  <div className="col-md-6 mb-3">
                    <label className="form-label small fw-bold">Date Attending</label>
                    <select
                      className="form-select form-select-sm"
                      value={selectedDate || ""}
          ...
          {editSection === "events" ? (
                <>
                  ...
                  <div className="form-check mb-3">
                      Tradeshow profiles have to attend the tradeshow
                  </div>
